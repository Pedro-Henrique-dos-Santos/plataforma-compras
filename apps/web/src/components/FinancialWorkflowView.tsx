                ...
                <div className="kanban-card-list">
                  {laneCards.map((card) => (
                    <button className={`financial-card ${card.overdue ? 'overdue' : ''}`} key={card.id} onClick={() => openCard(card)} type="button">
                      ...
                      <span className="financial-card-meta"><FileCheck2 size={14} />{card.invoiceNumbers.length ? `NF ${card.invoiceNumbers.join(', ')}` : 'Sem nota conciliada'}</span>
        ...
        <div className="modal-backdrop" role="presentation">
          <section aria-labelledby="financial-title" aria-modal="true" className="modal-panel modal-wide" role="dialog">
            ...
            <footer className="modal-actions">
              ...
              {editorMode === 'DETAIL' && canWrite && selected.stage === 'MATCHING_REQUIRED' && selected.instruction && (selected.advancePayment ? selected.hasAdvanceEvidence : selected.received && selected.invoiceNumbers.length > 0) && (settings?.approvalMode !== 'DISABLED' || selected.advancePayment) && (
